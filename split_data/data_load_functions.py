...
def path_from_inputs(splits, lin_log2, feat_sel='none'):
    ...
    if feat_sel != "none": # if feature selection method specified, include in path
        dir_path = os.path.join(base_dir, split_str, lin_log2, feat_sel_type[feat_sel])
    else:
        dir_path = os.path.join(base_dir, split_str, lin_log2)
...
def data_from_inputs(splits, lin_log2, feat_sel='none'):
    # These are initialized for paths later
    feat_sel_type = {'none': '', 'L1': 'lasso_subsets', 'L2': 'ridge_subsets', 'enet': 'elastic_net_subsets'}
    base_dir = os.getcwd()
    split_str = f'{splits}_fold'
